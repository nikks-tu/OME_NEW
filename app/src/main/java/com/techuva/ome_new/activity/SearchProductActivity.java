...
public class SearchProductActivity extends Fragment implements SearchProductAdapter.EventListener {
    ...
    String userId, searchKey, companyId, pagePerCount;
    int pageNumber;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        rcv_product_search_list.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                super.onScrollStateChanged(recyclerView, newState);
                if(rcv_product_search_list.SCROLL_STATE_DRAGGING==newState){
                    //fragProductLl.setVisibility(View.GONE);
                }
                if(rcv_product_search_list.SCROLL_STATE_IDLE==newState){
                        ...
                        if(pageNumber<listCount)
                        {
                            ...
                            if(!searchKey.equals(""))
                            {
                                if(userType.equals(Constants.Shopper)){
                                    showLoaderNew();
                                    serviceCallforSearchProducts(true);
                                ...
                                if(userType.equals("CONSU")){
                                    serviceCallforSearchProductsForConsumer(true);
                                    ll_bottom_view.setVisibility(View.VISIBLE);
                                    MApplication.serviceCallforCartProducts(searchContext, userId, accessToken);
                                    tv_company_name.setText(MApplication.getString(searchContext, Constants.SelectedCompany));
                                    tv_items_in_cart.setText(MApplication.getString(searchContext, Constants.CartSize));
      ...
      /*  rcv_product_search_list.setOnScrollChangeListener(new View.OnScrollChangeListener() {
            @Override
...
*/
    ...
    private void serviceCallforSearchProducts(final boolean more){
        ...
        Call<SearchProductMainObject> call = service.getAllProductForShopperWithSession(Integer.parseInt(userId), accessToken,
                new SearchProductPostParameters(Integer.parseInt(userId), Integer.parseInt(companyId), searchKey,  pagePerCount, String.valueOf(pageNumber)));
